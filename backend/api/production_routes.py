...
async def validate_data(
    ...
    if project_id:
        ...
        _, metadata, _ = manager.load_model(str(current_user.id), project_id, version_id)
        ...
        session = session_store.get(str(current_user.id))
...
async def get_global_explanation(
    ...
    try:
        model, metadata, _ = manager.load_model(str(current_user.id), project_id, version_id)
    except Exception as e:
        raise HTTPException(status_code=404, detail=f"Model not found: {str(e)}")
