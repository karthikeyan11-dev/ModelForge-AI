...
from models.user import User  # Ensure User is in metadata before create_all
from models.dataset import Dataset # Ensure Dataset is in metadata before create_all
from api.auth import get_current_user
from api.auth_routes import router as auth_router

# Create database tables for authentication (resilient startup with retries for Supabase/Cloud DB)
import time
from sqlalchemy.exc import OperationalError

print("🚀 Initializing Supabase cloud database...")
max_retries = 3
retry_delay = 5
for i in range(max_retries):
    try:
        ...
        if i == max_retries - 1:
            print(f"❌ Failed to reach Supabase after {max_retries} attempts: {e}")
            # We don't exit here to allow the server to start (for health checks), 
            # though DB-dependent routes will fail later.
        else:
            print(f"⚠️ Supabase not ready, retrying in {retry_delay}s... ({i+1}/{max_retries})")
...
if AUTOML_AVAILABLE:
    ...
    async def global_exception_handler(request: Request, exc: Exception):
        import logging
        import traceback
        logging.getLogger(__name__).error(f"UNHANDLED ERROR: {str(exc)}\n{traceback.format_exc()}")
        return JSONResponse(
            status_code=500,
            content={"detail": "A critical system error occurred. Our engineers have been notified. Please try again later."}
        )
