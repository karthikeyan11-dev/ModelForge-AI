...
def generate_narrative_explanation(local_explanation: Dict[str, Any]) -> str:
    """
    LLM-based natural language explanation for a local prediction.
    local_explanation should be the output of `explain_prediction()` natively converted to dict.
    """
    from services.ai_router import get_ai_router, DataSourceType
    ai_router = get_ai_router()
    
    top_positive = local_explanation.get('top_positive', [])[:3]
    top_negative = local_explanation.get('top_negative', [])[:3]
    
    try:
        score = float(local_explanation.get('prediction', 0.0))
        base = float(local_explanation.get('base_value', 0.0))
    except (ValueError, TypeError):
        score, base = 0.0, 0.0
        
    prompt = f"""
You are an AI assistant explaining a machine learning prediction to a non-technical end user.
Based on these underlying factors, explain why the prediction turned out this way.

Prediction Score: {score:.2f} 
Top factors increasing the score: {top_positive}
Top factors decreasing the score: {top_negative}

Rules:
- highly concise (2 sentences max)
- completely non-technical (do NOT use words like "SHAP", "base value", "features")
- easy to understand
Example: "The prediction is high mainly because of your higher income and education levels, though your age lowered it slightly."
"""
    try:
        model_conf = ai_router.get_model_for_source(DataSourceType.API)
        response = ai_router.client.get_text_completion(
            prompt=prompt,
        ...
        return "The prediction was driven primarily by your top active variables."

def compare_models_narrative(model1_meta: Dict[str, Any], model2_meta: Dict[str, Any]) -> str:
    """Compare two trained models structurally and generate a summarized narrative."""
    from services.ai_router import get_ai_router, DataSourceType
    ai_router = get_ai_router()
    
    prompt = f"""
Compare two machine learning models based on their AutoML results:

Model 1 ({model1_meta.get('name', 'Model A')}):
Metrics: {model1_meta.get('metrics', {})}
Key Features: {model1_meta.get('top_features', [])}

Model 2 ({model2_meta.get('name', 'Model B')}):
Metrics: {model2_meta.get('metrics', {})}
Key Features: {model2_meta.get('top_features', [])}

Explain why one performed better than the other in simple terms (e.g., capturing non-linear patterns, over/underfitting). Keep it short (2 sentences).
"""
    try:
        model_conf = ai_router.get_model_for_source(DataSourceType.API)
        response = ai_router.client.get_text_completion(
        ...
        logger.error(f"Failed to generate comparison narrative: {e}")
        return f"{model1_meta.get('name')} and {model2_meta.get('name')} identified overlapping patterns in your data."
