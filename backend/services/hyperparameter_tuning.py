...
# Optuna
# Existing Optuna logic is now integrated with MLflow
...
class HyperparameterTuner:
    ...
    def tune(
        ...
        # Log Tuning to MLflow
        experiment_id = mlflow_manager.get_or_create_experiment(experiment_name)
        
        with mlflow_manager.start_run(experiment_id=experiment_id, run_name=f"Tuning_{algorithm.value}"):
            mlflow.log_param("tuning_method", config.method.value)
            ...
            mlflow.log_metrics({
                ...
                "total_time": float(result.total_time_seconds)
            })
