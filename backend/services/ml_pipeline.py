...
import json
import mlflow
from .mlflow_utils import mlflow_manager
...
class MLPipeline:
    ...
    ALGORITHM_REGISTRY = {
        ...
        AlgorithmType.GRADIENT_BOOSTING_CLF: GradientBoostingClassifier,
        ...
        AlgorithmType.GRADIENT_BOOSTING_REG: GradientBoostingRegressor,
    }
    ...
    def train(
        ...
        try:
            ...
            # --- MLflow Logging (Audit Step 4 Readiness) ---
            with mlflow.start_run(run_name=algorithm_name, nested=True):
                metrics = {
                ...
                # Tags for selection logic and Audit Traceability
                tags = {
                    "algorithm": algorithm_name,
                ...
                # Inject metadata if provided via config or direct args
                if hasattr(config, "dataset_id"): tags["dataset_id"] = str(config.dataset_id)
                if hasattr(config, "user_id"): tags["user_id"] = str(config.user_id)
                
                mlflow_manager.log_training_run(
                    model=model,
                    algorithm_name=algorithm_name,
                    params=config.hyperparameters or self.DEFAULT_HYPERPARAMETERS.get(config.algorithm, {}),
                    metrics=metrics,
    ...
    def train_multiple(
        ...
        """
        Train multiple algorithms and compare, logging all to MLflow.
        """
        results = []
        
        # 1. Setup multi-tenant tracking context
        if user_id:
            mlflow_manager.set_active_experiment(str(user_id))
        else:
            mlflow.set_experiment(experiment_name)
        
        # Parent run for the AutoML session
        with mlflow.start_run(run_name=f"AutoML_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
            mlflow.set_tag("pipeline_type", "AutoML")
            mlflow.log_param("num_algorithms", len(algorithms))
            if dataset_id: mlflow.set_tag("dataset_id", str(dataset_id))
            if user_id: mlflow.set_tag("user_id", str(user_id))
            ...
            for algorithm in algorithms:
                ...
                result = self.train(X, y, config, experiment_id=experiment_id, preprocessing_pipeline=preprocessing_pipeline)
                results.append(result)
            
            # Tag the best model in MLflow after loop completes
            if self._best_model:
                mlflow.set_tag("best_model_algorithm", self._best_model.algorithm)
                # To tag the actual sub-run as 'best', we'd need to find its run_id, 
                # but tagging the parent works for high-level tracking.
